...
interface IzumiSwap {
    ...
    function swapAmount(Swap.SwapAmountParams calldata params) external  returns (uint256 cost, uint256 acquire);
}
...
contract Swap {
    ...
    address public _izumi_addr = 0xBd3bd95529e0784aD973FD14928eEDF3678cfad8;
    address _WETH = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    constructor() public {
    ...
    function profit_swap_u2bnb(SwapAmountParams calldata params,SwapAmountParams2 calldata params2) public {
        // izumi:busd->wbnb amm:wbnb->busd
        if (params2.direction == 0) {
            (uint256 ret,uint256 acquire) = IzumiSwap(_izumi_addr).swapAmount(params);
            require(params2.to == _owner);
            IAmmSwap(params2.amm_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                acquire,
                params2.amountOutMin,
                params2.path,
                params2.to,
                params2.deadline);
        } else {
            // amm:busd->bnb izumi:bnb->busd
            uint[] memory amounts_ret = IAmmSwap(params2.amm_router).swapExactTokensForETH(
                params2.amountIn,
                params2.amountOutMin,
                params2.path,
                params2.to,
                params2.deadline);
            require(params.recipient == _owner);
            params.amount = amounts_ret[0];
            (uint256 ret2,uint256 acquire2) = IzumiSwap(_izumi_addr).swapAmount(params);
        }
    }
